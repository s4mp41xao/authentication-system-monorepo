...
async function migrate() {
  ...
  try {
    ...
    const legacyInfluencers = await db
      .collection('influencerProfile')
      .find({})
      .toArray();
    console.log(
      `👤 Encontrados ${legacyInfluencers.length} perfis de influencer (legacy)`,
    );
    ...
    for (const legacy of legacyInfluencers) {
      try {
        ...
        console.error(
          `⚠️  Falha ao migrar influencer ${legacy?.email || legacy?.userId}:`,
          err,
        );
      }
    }
    ...
    console.log(
      `✅ Influencers: ${createdInf} criados, ${updatedInf} atualizados`,
    );

    // 2) Brands: brandProfile -> brands
    const legacyBrands = await db.collection('brandProfile').find({}).toArray();
    console.log(
      `🏢 Encontrados ${legacyBrands.length} perfis de brand (legacy)`,
    );
    ...
    for (const legacy of legacyBrands) {
      try {
        ...
        console.error(
          `⚠️  Falha ao migrar brand ${legacy?.email || legacy?.userId}:`,
          err,
        );
      }
    }
    ...
    console.log(
      `✅ Brands: ${createdBrand} criados, ${updatedBrand} atualizados`,
    );
